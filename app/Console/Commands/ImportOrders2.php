...
class ImportOrders2 extends Command {
	/**
	 * The name and signature of the console command.
	 *
	 * @var string
	 */
	protected $signature = 'rm:importOrders2';
	...
	public function handle() {
		...
		$csv = Reader::createFromPath( storage_path( 'Orders-2017-new3.csv' ), 'r' );
...
//			dd($srcOrder[ "Order Date" ]);

			//echo 'Magento_ID::' . $srcOrder[ "Magento ID" ];

//			$tmporderid = $this->getOldOrderID(100569378);
...
//				]
//			] );
//			for ( $tmppage = 350; $tmppage <= 1612; $tmppage++ ) {
//				$args = [
//					'filters' => [
...
//			dd('end');
			...
			$shopOrder = [
				...
				"tags" => "magento_order_number::{$srcOrder[ "Magento ID" ]},Oneill::fulfilled,Oneill::Pulled,Riskified::approved",
			];
...
#					"fulfillment_status" => "fulfilled",
			...
			//dd( $this->getOldOrderID((int)$srcOrder[ "Magento ID" ]) );
			//$this->getOldOrderID((int)$srcOrder[ "Magento ID" ]);
			$oldorder_id = $this->getOldOrderID((int)$srcOrder[ "Magento ID" ]);

			//dd($oldorder_id);

			if ( !is_null($oldorder_id) ) {
				//dd($oldorder_id);
				...
				echo "No Magento ID Record - ".(int)$srcOrder[ "Magento ID" ]."\n";
			}
			return true;
		} );
	...
	private function getOldOrderID($magid) {
		...
		$getShopify = DB::table('shopify_json_orders2')
		...
		if (!is_null($getShopify)) {
			//$textval = json_decode($getShopify->text);

			//return $textval->id;
			return $getShopify->shopify_id;
		} else {
			return null;
		}
	}
